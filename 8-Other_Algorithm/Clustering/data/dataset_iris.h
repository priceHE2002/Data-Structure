/**
 * dataset_iris.h — UCI Iris 数据集
 * 150 样本 | 4 特征 | 3 类别
 * 特征: 花萼长, 花萼宽, 花瓣长, 花瓣宽
 * 类别: 0=Iris-setosa, 1=Iris-versicolor, 2=Iris-virginica
 */
#ifndef DATASET_IRIS_H
#define DATASET_IRIS_H

const int IRIS_DIM   = 4;
const int IRIS_SIZE  = 150;
const int IRIS_CLASSES = 3;

void load_iris(double features[IRIS_SIZE][IRIS_DIM], int labels[IRIS_SIZE]) {
    const double raw[IRIS_SIZE][IRIS_DIM + 1] = {
...
{6.2,3.4,5.4,2.3,2},{5.9,3.0,5.1,1.8,2},
    ...
    for (int i = 0; i < IRIS_SIZE; i++) {
        for (int j = 0; j < IRIS_DIM;  j++) features[i][j] = raw[i][j];
        labels[i] = (int)raw[i][IRIS_DIM];
    }
}
#endif
